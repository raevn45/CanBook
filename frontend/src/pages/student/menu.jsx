...
export default function Menu() {
  ...
  return (
    <main className="app-page menu-page-new">
      ...
      {loading ? <div className="loading-card">Loading today's menu…</div> : <div className="live-menu-grid">{filtered.map((item, index) => <motion.article key={item.item_id} className="live-menu-card" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.045 }} whileHover={{ y: -7 }}><div className="menu-card-top"><span>0{index + 1}</span><small>{item.category || "canteen"}</small></div><div className="food-mark">{String(item.item_name).slice(0, 1)}</div><h2>{item.item_name}</h2><p>{item.description || "Freshly prepared at the school canteen."}</p><div className="live-menu-bottom"><strong>AED {Number(item.price).toFixed(2)}</strong><button onClick={() => addToCart(item)} disabled={item.available === false}>Add <ArrowUpRight size={16} /></button></div></motion.article>)}</div>}
