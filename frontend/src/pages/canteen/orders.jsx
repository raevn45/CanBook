...
export default function Orders() {
  ...
  const deleteExistingOrder = async (id) => {
    ...
    try {
      ...
      setData((current) => ({ ...current, orders: current.orders.filter((order) => order.order_id !== id) }));
  ...
  return (
    <main className="app-page canteen-orders-page">
      ...
      {!error && data.orders.length > 0 && <section className="queue-board"><div className="queue-board-head"><span>{data.orders.length} active order{data.orders.length === 1 ? "" : "s"}</span><span>LIVE CANTEEN BOARD</span></div><motion.div layout className="queue-grid"><AnimatePresence mode="popLayout">{data.orders.map((order, index) => {
        const isConfirming = confirmDelete === order.order_id;
        return <motion.article layout key={order.order_id} className={`queue-card status-${statusClass(order.status)}`} initial={{ opacity: 0, y: 25, scale: .96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: .88 }} transition={{ delay: Math.min(index * .05, .25), type: "spring", stiffness: 240, damping: 22 }} whileHover={{ y: -7 }}>
          ...
          <div className="queue-delete-row">
            <AnimatePresence mode="wait" initial={false}>
              {isConfirming ? <motion.div className="order-delete-confirm" initial={{ opacity: 0, scale: .94 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: .94 }}><span>Delete?</span><button type="button" onClick={() => deleteExistingOrder(order.order_id)} disabled={deleting === order.order_id}>{deleting === order.order_id ? "Deleting…" : "Delete order"}</button><button type="button" onClick={() => setConfirmDelete(null)} aria-label="Cancel delete"><X size={15} /></button></motion.div> : <motion.button type="button" className="order-delete-button" onClick={() => setConfirmDelete(order.order_id)} whileHover={{ y: -2 }} whileTap={{ scale: .94 }} aria-label={`Delete order ${order.order_id}`}><Trash2 size={16} /></motion.button>}
            </AnimatePresence>
          </div>
        </motion.article>;
